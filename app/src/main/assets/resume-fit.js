...
let pendingFitCards = new Set();
let fitRaf = 0;
function scheduleFit(card){
  pendingFitCards.add(card);
  if (!fitRaf){
    fitRaf = requestAnimationFrame(()=>{
      fitRaf = 0;
      for (const c of pendingFitCards){
        fitApply(c);
      }
      pendingFitCards.clear();
    });
  }
}
...
if (updateBtnEl) {
  bindTapDown(updateBtnEl, performSmartUpdate);
}

// ── アクティブモーダルの閉鎖・Android戻るキー/Escape連携 ──
window.closeActiveModal = function(){
  const nameEditOverlayEl = document.getElementById('nameEditOverlay');
  if (nameEditOverlayEl && nameEditOverlayEl.classList.contains('show')){
    closeNameEditPopup();
    return true;
  }
  const colorPickerOverlayEl = document.getElementById('colorPickerOverlay');
  if (colorPickerOverlayEl && colorPickerOverlayEl.classList.contains('show')){
    closeColorPickerPopup();
    return true;
  }
  if (typeof setupPanelEl !== 'undefined' && setupPanelEl && setupPanelEl.classList.contains('show')){
    closeSetup();
    if (typeof pendingAddGroupId !== 'undefined') pendingAddGroupId = null;
    if (typeof pendingInsertAfterId !== 'undefined') pendingInsertAfterId = null;
    return true;
  }
  if (typeof addPanelEl !== 'undefined' && addPanelEl && addPanelEl.classList.contains('show')){
    closeAddPanel();
    return true;
  }
  if (typeof toastEl !== 'undefined' && toastEl && toastEl.classList.contains('show')){
    closeToast();
    return true;
  }
  if (typeof movingItemId !== 'undefined' && movingItemId != null){
    cancelMove();
    return true;
  }
  if ((typeof claimId !== 'undefined' && claimId != null) || (typeof pending40Id !== 'undefined' && pending40Id != null)){
