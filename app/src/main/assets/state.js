...
function sanitizeItem(it){
  ...
  const now = Date.now();
  if (typeof it.start !== 'number' || !Number.isFinite(it.start)) it.start = now;
  if (it.type === 'stam'){
    it.intervalMin = Math.max(1, clampInt(it.intervalMin, 1, 99999, 5));
    it.max = Math.max(1, clampInt(it.max, 1, 999, 100));
    it.current = Math.max(0, Math.min(it.max, clampInt(it.current, 0, 999, it.max)));
  } else if (it.type === 'orb'){
    it.intervalMin = Math.max(1, clampInt(it.intervalMin, 1, 99999, 360));
    it.max = Math.max(1, clampInt(it.max, 1, 99, 4));
    it.current = Math.max(0, Math.min(it.max, clampInt(it.current, 0, 99, it.max)));
    if (!it.orbMode) it.orbMode = 'down';
  } else if (it.type === 'idle' || it.type === 'exped'){
    it.durationMin = Math.max(1, clampInt(it.durationMin, 1, 99999, 60));
      ...
      if (data && Array.isArray(data.items)){
        data.items = data.items.map(it => {
          ...
          if (it.type === 'group'){
            ...
            it.children = it.children.map(ch => sanitizeItem(ch)).filter(Boolean);
            return it;
          }
